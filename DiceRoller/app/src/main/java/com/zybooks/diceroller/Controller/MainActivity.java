...
public class MainActivity extends AppCompatActivity
    ...
    private long mTimerLength = 1000;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Bottom TextViews in Constraint Layout
        mTotalSumTextView = findViewById(R.id.totalSumTextView);
        ...
        LinearLayout linearLayout = findViewById(R.id.mainActivityLayout);
        mDetector = new GestureDetectorCompat(linearLayout.getContext(), new DiceGestureListener());
    }
    ...
    private void showDice() {
        ...
        for (int i = 0; i < mVisibleDice; i++) {
            ...
            //add to total.
            mTotalSum = mTotalSum + mDice[i].getNumber();
        }
        //show Total
        mTotalSumTextView.setText(Integer.toString(mTotalSum));
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        else if (item.getItemId() == R.id.action_stop) { //stops dice roll
            mTimer.cancel();
    ...
    //rolls the dice upon click of the roll/shuffle icon.
    private void rollDice() {
        mMenu.findItem(R.id.action_stop).setVisible(true); //makes stop icon visible

        if (mTimer != null) {
            mTimer.cancel();
        }

        mTimer = new CountDownTimer(mTimerLength, 140) {
        ...
        //Checks if you win or lose the game
                /*
                    2 Dice:
                        ...
                        LOSE if sum is 2 or 12.

                    3 Dice:
                        WIN if sum is divisible by 7 or 11.
                        LOSE if sum is 3 or 18.
                 */
        //mTotalSum = 2; my own checker to see if the user wins or loses.
        if (mVisibleDice == 2){
            if (mTotalSum == 7 || mTotalSum == 11){
                mStatusTextView.setVisibility(View.VISIBLE);
                mStatusTextView.setText("WON");
            } else if (mTotalSum == 2 || mTotalSum == 12){
                mStatusTextView.setVisibility(View.VISIBLE);
                mStatusTextView.setText("LOST");
            }
        } else if (mVisibleDice == 3){
            if (mTotalSum % 7 == 0 || mTotalSum % 11 == 0){
                mStatusTextView.setVisibility(View.VISIBLE);
                mStatusTextView.setText("WON");
            } else if (mTotalSum == 3 || mTotalSum == 18){
                mStatusTextView.setVisibility(View.VISIBLE);
                mStatusTextView.setText("LOST");
            }
        }
